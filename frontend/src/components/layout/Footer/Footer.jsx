...
const Footer = () => {
  const { botStatus, wsConnectionStatus, marketData } = useTrading();
  ...
  return (
    <footer className="bg-gray-900 border-t border-gray-800 px-6 py-3 md:px-12">
      <div className="max-w-6xl mx-auto flex flex-col md:flex-row justify-between items-center gap-4">
        ...
        {/* Right: version + last symbol */}
        <div className="flex items-center gap-4">
          <span className="text-gray-500 text-sm font-mono">v1.0.0</span>
          {marketData?.symbol && (
            <span className="text-gray-400 text-sm font-mono">• Trading: {marketData.symbol}</span>
          )}
        </div>
      </div>
    </footer>
